...
// @namespace          pionxzh, GeradeHouse
// @version            2.30.0
// @author             pionxzh, GeradeHouse
...
System.register("./__monkey.entry-BlCgl1t6.js", ['jszip', 'html2canvas'], (function (exports, module) {
  ...
  return {
    ...
    execute: (function () {
      ...
      const KEY_IMAGE_HANDLING_STRATEGY = "exporter:image_handling_strategy";
      const KEY_IMAGE_CUSTOM_MARKER = "exporter:image_custom_marker";
      const KEY_IMAGE_QUALITY = "exporter:image_quality";
      const KEY_IMAGE_MAX_SIZE = "exporter:image_max_size";
      const KEY_IMAGE_INCLUDE_METADATA = "exporter:image_include_metadata";
      var _GM_deleteValue = /* @__PURE__ */ (() => typeof GM_deleteValue != "undefined" ? GM_deleteValue : void 0)();
      var _GM_getValue = /* @__PURE__ */ (() => typeof GM_getValue != "undefined" ? GM_getValue : void 0)();
      var _GM_setValue = /* @__PURE__ */ (() => typeof GM_setValue != "undefined" ? GM_setValue : void 0)();
      var _unsafeWindow = /* @__PURE__ */ (() => typeof unsafeWindow != "undefined" ? unsafeWindow : void 0)();
      function getBase64FromImg$1(el) {
      ...
      async function getBase64FromImageUrl$1(url) {
        const img = await loadImage$1(url);
        return getBase64FromImg$1(img);
      }
      ...
      async function getUserAvatar() {
        try {
          ...
          if (picture) return await getBase64FromImageUrl$1(picture);
        } catch (e2) {
          console.error(e2);
        }
        try {
          ...
          if (avatar) return getBase64FromImg$1(avatar);
        } catch (e2) {
          console.error(e2);
        }
      ...
      const en_US = {
        ...
        "Image Handling Description": "Configure how images are handled in exports",
        "Embed Images": "Embed images (current behavior)",
        "Embed Images Description": "Images are embedded as base64 data within the content file",
        "Text Markers": "Replace with text markers",
        "Text Markers Description": "Images are replaced with customizable text placeholders",
        "Separate Files": "Separate files in ZIP archive",
        "Separate Files Description": "Images are saved as separate files with relative links in a ZIP package",
        "Advanced Options": "Advanced Options",
        "Image Quality": "Image Quality",
        "Image Quality Description": "Compression quality for images (higher = better quality, larger file size)",
        "Max Image Size": "Max Image Size",
        "Max Image Size Description": "Maximum width or height for images in pixels",
        "Include Image Metadata": "Include image metadata",
        "Include Image Metadata Description": "Include detailed image information in export metadata",
        "Image Handling Preview": "Preview",
        "Custom Marker Text": "Custom marker text",
        "Custom Marker Text Description": "Text to replace images when using text markers option",
        "Processing Images": "Processing images...",
        "Image Processing Complete": "Image processing complete",
      ...
      function head(node2) {
        ...
        while (++index2 < children.length) {
          ...
          if (child.type === "element" && (child.tagName === "title" || child.tagName === "base")) {
            if (seen2.includes(child.tagName)) return false;
            seen2.push(child.tagName);
          }
        }
...
\f\r "&'/<=>`.split(""), "\0	\n\f\r \"&'/<=>`".split("")]
      ...
      function getMimeType(dataUrlOrUrl) {
        var _a;
        if (dataUrlOrUrl.startsWith("data:")) {
          const match = dataUrlOrUrl.match(/data:([^;]+);/);
          return match ? match[1] : "image/png";
        }
        const extension2 = (_a = dataUrlOrUrl.split(".").pop()) == null ? void 0 : _a.toLowerCase();
        switch (extension2) {
          case "jpg":
          case "jpeg":
            return "image/jpeg";
      ...
      function generateImageId(context) {
        const timestamp2 = context.timestamp || Date.now();
        const hash = btoa(`${context.conversationId}-${context.messageId}-${context.imageIndex}-${timestamp2}`);
        return hash.replace(/[^a-zA-Z0-9]/g, "").substring(0, 16);
      }
      function generateImageFileName(context, extension2 = "png") {
        ...
        return `${prefix}-${paddedIndex}.${extension2}`;
      }
      ...
      function getFileSizeFromBase64(base64) {
        const base64Data = base64.split(",")[1];
        return Math.ceil(base64Data.length * 3 / 4);
      }
      function sanitizeFileName(fileName) {
        return fileName.replace(/[<>:"/\\|?*]/g, "_").replace(/\s+/g, "-").toLowerCase();
      }
      function createImageRelativePath(fileName) {
      ...
      class EmbedBase64Strategy extends BaseImageProcessor {
        ...
        async processImage(imageUrl, context) {
          ...
          try {
            ...
            console.warn(`Failed to process image ${imageUrl}:`, error2);
            return {
              id,
      ...
      class SeparateFilesStrategy extends BaseImageProcessor {
        ...
        async processImage(imageUrl, context) {
          ...
          try {
            const base64Data = await getBase64FromImageUrl(imageUrl);
            const extension2 = mimeType.split("/")[1] || "png";
            const fileName = sanitizeFileName(generateImageFileName(context, extension2));
      ...
      function createImageProcessor(strategy) {
        switch (strategy) {
          case "embed_base64":
      ...
      class DefaultImageHandler {
        constructor(initialStrategy) {
          ...
          __publicField(this, "currentStrategy");
         ...
         */
        async processConversationImages(images, format) {
          const processedImages = await Promise.all(
            images.map(async ({ url, context }) => {
              try {
                return await this.processor.processImage(url, context);
              } catch (error2) {
                console.warn(`Failed to process image ${url}:`, error2);
                return {
        ...
        async generateContentFromImages(processedImages, format) {
          ...
          if (this.currentStrategy === "separate_files") {
            processedImages.forEach((image2) => {
              if (image2.originalData && image2.fileName && image2.metadata) {
                const blob = base64ToBlob(image2.originalData, image2.metadata.mimeType);
                files.push({
                  path: image2.content,
                  // This is the relative path
                  data: blob,
                  mimeType: image2.metadata.mimeType
                });
              }
            });
          }
        ...
        generateExportMetadata(processedImages) {
          ...
          return {
            ...
            // Will be filled by export function
         ...
         */
        createImageContext(imageData, conversationId, messageId, imageIndex, author, timestamp2) {
         ...
         */
        getMimeTypeFromImageData(imageData) {
          var _a;
          if (imageData.image_url) {
            const url = imageData.image_url;
            const extension2 = (_a = url.split(".").pop()) == null ? void 0 : _a.toLowerCase();
            switch (extension2) {
              case "jpg":
              case "jpeg":
                return "image/jpeg";
         ...
         */
        replaceImagePlaceholders(content2, processedImages, format) {
          let result = content2;
          processedImages.forEach((image2, index2) => {
            const placeholder = `[IMAGE_${index2}]`;
            let replacement = "";
            switch (format) {
              case "markdown":
                if (this.currentStrategy === "text_marker") {
                  replacement = image2.content;
                } else if (this.currentStrategy === "embed_base64") {
                  replacement = `![image](${image2.content})`;
              ...
              case "html":
                if (this.currentStrategy === "text_marker") {
                  replacement = `<div class="image-placeholder">${image2.content}</div>`;
                } else if (this.currentStrategy === "embed_base64") {
                  replacement = `<img src="${image2.content}" />`;
                } else if (this.currentStrategy === "separate_files") {
                  replacement = `<img src="${image2.content}" />`;
                }
                break;
      ...
      function initializeImageHandler(strategy) {
      ...
      async function exportToHtml(fileNameFormat, metaList) {
        ...
        const imageHandlingStrategy = ScriptStorage.get(KEY_IMAGE_HANDLING_STRATEGY) || "embed_base64";
        initializeImageHandler(imageHandlingStrategy);
        ...
        if (imageHandlingStrategy === "separate_files" && exportFiles && exportFiles.length > 0) {
          ...
          if (metadata) {
            ...
            metadata.settings = {
              imageQuality: ScriptStorage.get(KEY_IMAGE_QUALITY) || 85,
              maxImageSize: ScriptStorage.get(KEY_IMAGE_MAX_SIZE) || 2048,
              includeImageMetadata: ScriptStorage.get(KEY_IMAGE_INCLUDE_METADATA) || true,
              customMarkerText: ScriptStorage.get(KEY_IMAGE_CUSTOM_MARKER) || "[Image Omitted]"
            };
          }
      ...
      async function exportAllToHtml(fileNameFormat, apiConversations, metaList) {
        ...
        for (const conversation of conversations) {
          ...
          const { html: html2 } = await conversationToHtml(conversation, userAvatar, metaList);
          zip.file(fileName, html2);
        }
      ...
      async function conversationToHtml(conversation, avatar, metaList) {
        var _a, _b, _c, _d;
        ...
        const images = extractImagesFromConversation$1(conversation);
        let processedImages = [];
        let exportFiles = [];
        let imageMetadata;
        if (images.length > 0) {
        ...
        for (const { message } of conversationNodes) {
          ...
          const messageContent = await transformContent$2(message.content, message.metadata, postProcess, processedImages, imageIndex);
          imageIndex += countImagesInMessage$1(message);
...
<div class="conversation-item">
    ...
    <div class="metadata_container">
        ...
        const html2 = templateHtml.replaceAll("{{title}}", title2).replaceAll("{{date}}", date).replaceAll("{{time}}", time).replaceAll("{{source}}", source).replaceAll("{{lang}}", lang).replaceAll("{{theme}}", theme).replaceAll("{{avatar}}", avatar).replaceAll("{{details}}", detailsHtml).replaceAll("{{content}}", conversationHtml.join("\n\n"));
        return {
          html: html2,
          exportFiles,
          imageMetadata
      ...
      function extractImagesFromConversation$1(conversation) {
        const images = [];
        conversation.conversationNodes.forEach((node2, nodeIndex) => {
          var _a, _b;
          const message = node2.message;
          if (!message || !message.content) return;
          let imageIndex = 0;
          if (message.content.content_type === "execution_output" && ((_b = (_a = message.metadata) == null ? void 0 : _a.aggregate_result) == null ? void 0 : _b.messages)) {
      ...
      function countImagesInMessage$1(message) {
        var _a, _b;
      ...
      async function transformContent$2(content2, metadata, postProcess, processedImages, imageStartIndex) {
        var _a, _b, _c, _d;
          ...
          case "execution_output":
            if ((_b = metadata == null ? void 0 : metadata.aggregate_result) == null ? void 0 : _b.messages) {
              ...
              if (imageMessages.length > 0) {
                const imageContent = imageMessages.map((_24, index2) => {
                  const globalIndex = imageStartIndex + index2;
...
\`\`\`` || "");
          ...
          case "multimodal_text": {
            const parts = ((_d = content2.parts) == null ? void 0 : _d.map((part, partIndex) => {
              var _a2;
              if (typeof part === "string") return postProcess(part);
              if (part.content_type === "image_asset_pointer") {
                const imageIndexInMessage = ((_a2 = content2.parts) == null ? void 0 : _a2.slice(0, partIndex).filter(
                  (p2) => typeof p2 === "object" && p2.content_type === "image_asset_pointer"
                ).length) || 0;
      ...
      function extractImagesFromConversation(conversation) {
        const images = [];
        conversation.conversationNodes.forEach((node2, nodeIndex) => {
          var _a, _b;
          const message = node2.message;
      ...
      async function conversationToMarkdown(conversation, metaList) {
        var _a, _b, _c;
        const { id, title: title2, model, modelSlug, createTime, updateTime, conversationNodes } = conversation;
        const source = `${baseUrl}/c/${id}`;
        const _metaList = (metaList == null ? void 0 : metaList.filter((x2) => !!x2.name).map(({ name, value }) => {
...
`;
          ...
          const postProcess = (input) => postSteps.reduce((acc, fn2) => fn2(acc), input);
          const messageContent = await transformContent$1(message.content, message.metadata, postProcess, processedImages, imageIndex);
          imageIndex += countImagesInMessage(message);
          content2.push(`#### ${author}:
...
${citationText}`;
      ...
      async function transformContent$1(content2, metadata, postProcess, processedImages, imageStartIndex) {
        var _a, _b, _c, _d;
        switch (content2.content_type) {
          case "text":
            return postProcess(((_a = content2.parts) == null ? void 0 : _a.join("\n")) || "");
...
${content2}`;
      ...
      const defaultImageHandlingStrategy = "embed_base64";
      ...
      const SettingContext = G$1({
        ...
        setImageHandlingStrategy: (_24) => {
        },
        imageCustomMarker: defaultCustomMarkerText,
        setImageCustomMarker: (_24) => {
        },
              ...
              /* @__PURE__ */ o$8($5d3850c4d0b4e6c7$export$602eac185826482c, { children: [
                ...
                /* @__PURE__ */ o$8($5d3850c4d0b4e6c7$export$7c6e2c02157bb7d2, { className: "DialogContent", children: [
                  ...
                  /* @__PURE__ */ o$8("dl", { className: "space-y-6", children: [
                    ...
                    /* @__PURE__ */ o$8("div", { className: "relative flex bg-white dark:bg-white/5 rounded p-4", children: /* @__PURE__ */ o$8("div", { children: [
                      /* @__PURE__ */ o$8("dt", { className: "text-md font-medium text-gray-800 dark:text-white", children: t2("Image Handling") }),
                      /* @__PURE__ */ o$8("dd", { className: "text-sm text-gray-700 dark:text-gray-300", children: [
                        t2("Image Handling Description"),
                        /* @__PURE__ */ o$8("div", { className: "mt-4 space-y-3", children: [
                          /* @__PURE__ */ o$8("div", { className: "flex items-start", children: [
                            /* @__PURE__ */ o$8(
                              "input",
                              {
                                type: "radio",
                                ...
                                onChange: (e2) => setImageHandlingStrategy(e2.currentTarget.value),
                                className: "mt-1 mr-3"
                              }
                            ),
                            /* @__PURE__ */ o$8("div", { children: [
                              /* @__PURE__ */ o$8("label", { htmlFor: "embed-base64", className: "font-medium text-gray-800 dark:text-white", children: t2("Embed Images") }),
                              /* @__PURE__ */ o$8("p", { className: "text-xs text-gray-600 dark:text-gray-400", children: t2("Embed Images Description") })
                            ] })
                          ] }),
                          /* @__PURE__ */ o$8("div", { className: "flex items-start", children: [
                            /* @__PURE__ */ o$8(
                            ...
                            /* @__PURE__ */ o$8("div", { children: [
                              /* @__PURE__ */ o$8("label", { htmlFor: "text-marker", className: "font-medium text-gray-800 dark:text-white", children: t2("Text Markers") }),
                              /* @__PURE__ */ o$8("p", { className: "text-xs text-gray-600 dark:text-gray-400", children: t2("Text Markers Description") }),
                              imageHandlingStrategy === "text_marker" && /* @__PURE__ */ o$8("div", { className: "mt-2", children: [
                                /* @__PURE__ */ o$8("label", { className: "text-xs font-medium text-gray-700 dark:text-gray-300", children: t2("Custom Marker Text") }),
                                /* @__PURE__ */ o$8(
                                  "input",
                                  {
                                    type: "text",
                                    ...
                                    onChange: (e2) => setImageCustomMarker(e2.currentTarget.value),
                          ...
                          /* @__PURE__ */ o$8("div", { className: "flex items-start", children: [
                            ...
                            /* @__PURE__ */ o$8("div", { children: [
                              /* @__PURE__ */ o$8("label", { htmlFor: "separate-files", className: "font-medium text-gray-800 dark:text-white", children: t2("Separate Files") }),
                              /* @__PURE__ */ o$8("p", { className: "text-xs text-gray-600 dark:text-gray-400", children: t2("Separate Files Description") })
                            ] })
                          ] })
                        ] }),
                        imageHandlingStrategy === "separate_files" && /* @__PURE__ */ o$8("div", { className: "mt-4 p-3 bg-gray-50 dark:bg-gray-800 rounded", children: [
                          /* @__PURE__ */ o$8("h4", { className: "text-sm font-medium text-gray-800 dark:text-white mb-3", children: t2("Advanced Options") }),
                          /* @__PURE__ */ o$8("div", { className: "mb-3", children: [
                            /* @__PURE__ */ o$8("label", { className: "text-xs font-medium text-gray-700 dark:text-gray-300", children: [
                              t2("Image Quality"),
                              ": ",
                              imageQuality,
                              "%"
                            ...
                            /* @__PURE__ */ o$8(
                              "input",
                              {
                                ...
                                max: "100",
                                value: imageQuality,
                                onChange: (e2) => setImageQuality(Number(e2.currentTarget.value)),
                                className: "w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700 mt-1"
                              }
                            ),
                            /* @__PURE__ */ o$8("p", { className: "text-xs text-gray-600 dark:text-gray-400 mt-1", children: t2("Image Quality Description") })
                          ] }),
                          /* @__PURE__ */ o$8("div", { className: "mb-3", children: [
                            /* @__PURE__ */ o$8("label", { className: "text-xs font-medium text-gray-700 dark:text-gray-300", children: [
                              t2("Max Image Size"),
                            ...
                            /* @__PURE__ */ o$8(
                              "input",
                              {
                                ...
                                step: "256",
                                value: imageMaxSize,
                                onChange: (e2) => setImageMaxSize(Number(e2.currentTarget.value)),
                                className: "w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700 mt-1"
                              }
                            ),
                            /* @__PURE__ */ o$8("p", { className: "text-xs text-gray-600 dark:text-gray-400 mt-1", children: t2("Max Image Size Description") })
                          ] }),
                          /* @__PURE__ */ o$8("div", { className: "flex items-center mt-2", children: [
                            /* @__PURE__ */ o$8(
                              "input",
                              {
                                ...
                                onChange: (e2) => setImageIncludeMetadata(e2.currentTarget.checked),
                                className: "mr-2"
                              }
                            ),
                            /* @__PURE__ */ o$8("label", { htmlFor: "include-image-metadata", className: "text-xs text-gray-700 dark:text-gray-300", children: t2("Include Image Metadata") })
                          ] }),
                          /* @__PURE__ */ o$8("p", { className: "text-xs text-gray-600 dark:text-gray-400 mt-1 ml-6", children: t2("Include Image Metadata Description") })
                        ] })
                      ] })
                    ] }) })
                  ] }),
...
System.register("./zip-packager-DbpWUpir-BoUwajsC.js", ['jszip', './__monkey.entry-BlCgl1t6.js', 'html2canvas'], (function (exports, module) {
  ...
  return {
    setters: [module => {
      ...
      sanitizeFileName = module.s;
    }, null],
    execute: (function () {

      exports({
        createExportZip: createExportZip,
...
`;
         ...
         */
        async generateZip(compressionLevel = 9) {
      ...
      function generateZipFileName(conversationTitle, strategy, timestamp) {
